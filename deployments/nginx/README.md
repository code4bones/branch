...
The React admin shell uses Ant Design 5. This local beta web origin therefore
permits `style-src 'self' 'unsafe-inline'` for AntD runtime style injection. Do
not add `unsafe-inline` to `script-src`; the relaxation is only for presentation
styles and does not alter relay or node-admin traffic.
