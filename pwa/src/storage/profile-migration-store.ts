...
// attachment handles, diagnostics, the device-local outbox, and received-ID
// dedup metadata never enter this transaction.
...
export async function replacePortableProfileSnapshot(snapshot: PortableProfileSnapshot): Promise<void> {
  ...
  try {
    await new Promise<void>((resolve, reject) => {
      ...
      for (const message of snapshot.messages) messages.put(message);
